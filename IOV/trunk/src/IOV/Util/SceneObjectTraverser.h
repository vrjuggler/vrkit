#ifndef _INF_UTIL_SCENE_OBJECT_TRAVERSER_H_
#define _INF_UTIL_SCENE_OBJECT_TRAVERSER_H_
...
/**
 * Basic traverser functions for IOV scene objects.
 *
 * @since 0.25.1
 */
class IOV_CLASS_API SceneObjectTraverser
{
public:
   /** Traversal results. */
   enum Result
   ...
   typedef boost::function<Result (inf::SceneObjectPtr)> enter_func_t;
   typedef boost::function<Result (inf::SceneObjectPtr, Result)> exit_func_t;

   /** @name Traversal Functions */
   //@{
   /**
    * Traverses the tree of IOV scene objects rooted at \p root using a
    ...
    */
   static Result traverse(inf::SceneObjectPtr root, enter_func_t enterFunc,
                          exit_func_t exitFunc = defaultExitFunc);

   /**
    * Traverses the trees of IOV scene objects rooted at \p roots using a
    ...
    *         returned to the caller.
    */
   static Result traverse(const std::vector<inf::SceneObjectPtr>& roots,
...
private:
   ...
   static Result defaultExitFunc(inf::SceneObjectPtr, Result r)
   {
      return r;
   }
};
...
#endif /* _INF_UTIL_SCENE_OBJECT_TRAVERSER_H_ */
