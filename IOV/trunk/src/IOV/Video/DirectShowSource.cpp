...
ByteStream::ByteStream(ByteSource *parent, const vpr::Uint32 width, const vpr::Uint32 height)
   : CBaseOutputPin(NAME("ByteStreamObj"), parent, parent->lock(), NULL, L"ByteStreamPin")
   , mFilter(parent)
   , mImageWidth(width)
   , mImageHeight(height)
   , mLastTime(0ll)
...
HRESULT ByteStream::DecideBufferSize(IMemAllocator *pAlloc, ALLOCATOR_PROPERTIES *pProperties)
{
...
void ByteStream::grabFrame(const vpr::Uint32 width, const vpr::Uint32 height, vpr::Uint8* data)
{
   IMediaSample* media_sample;
   CHECK_RESULT(GetDeliveryBuffer(&media_sample,NULL,NULL,0), "Failed to get a media sample.");
   ...
   media_sample->SetTime((REFERENCE_TIME *) &rtStart,(REFERENCE_TIME *) &mLastTime);
