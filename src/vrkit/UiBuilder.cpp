...
void splitStr(const std::string& s, const std::string& sep, OutIt dest)
{
...
OSG::MaterialPtr UiBuilder::createDefaultMaterial()
{
   ...
   OSG::CPEditor me(
      mat, 
      OSG::ChunkMaterial::ChunksFieldMask | OSG::ChunkMaterial::SortKeyFieldMask
   );
#endif
...
OSG::GeometryPtr UiBuilder::buildSphere(const OSG::UInt16 depth,
                                        const OSG::Real32 radius,
...
#else
   OSG::calcVertexTangents(geom, 0, OSG::Geometry::TexCoords1Index,
                           OSG::Geometry::TexCoords2Index);
#endif
   OSG::Color4f used_color(color.red(), color.green(), color.blue(), alpha);

   OSG::GeoPositions3fPtr verts =
#if OSG_MAJOR_VERSION < 2
      OSG::GeoPositions3fPtr::dcast(geom->getPositions());
#else
      OSG::cast_dynamic<OSG::GeoPositions3fPtr>(geom->getPositions());
#endif
   OSG::GeoColors4fPtr colors = OSG::GeoColors4f::create();
   OSG::GeoColors4f::StoredFieldType* mfc =
#if OSG_MAJOR_VERSION < 2
      colors->getFieldPtr();
#else
      colors->editFieldPtr();
...
{
   OSG::GeoPTypesPtr types =
#if OSG_MAJOR_VERSION < 2
      OSG::GeoPTypesUI8Ptr::dcast(geom->getTypes());
#else
      OSG::cast_dynamic<OSG::GeoPTypesUI8Ptr>(geom->getTypes());
#endif
   OSG::GeoPLengthsPtr lens =
#if OSG_MAJOR_VERSION < 2
      OSG::GeoPLengthsUI32Ptr::dcast(geom->getLengths());
#else
      OSG::cast_dynamic<OSG::GeoPLengthsUI32Ptr>(geom->getLengths());
#endif
   OSG::GeoPositions3fPtr verts =
#if OSG_MAJOR_VERSION < 2
      OSG::GeoPositions3fPtr::dcast(geom->getPositions());
      ...
      OSG::cast_dynamic<OSG::GeoPositions3fPtr>(geom->getPositions());
#endif
   OSG::GeoColors4fPtr colors =
#if OSG_MAJOR_VERSION < 2
      OSG::GeoColors4fPtr::dcast(geom->getColors());
#else
      OSG::cast_dynamic<OSG::GeoColors4fPtr>(geom->getColors());
#endif
   OSG::GeoNormals3fPtr norms =
#if OSG_MAJOR_VERSION < 2
      OSG::GeoNormals3fPtr::dcast(geom->getNormals());
#else
      OSG::cast_dynamic<OSG::GeoNormals3fPtr>(geom->getNormals());
#endif
                               ...
                               const float backDepth, const float alpha)
{
   ...
   OSG::Color4f used_color(color.red(), color.green(), color.blue(), alpha);
   // If front and back are the same, then only do front surface
   bool only_front_flag = frontDepth == backDepth;
                                      ...
                                      const float backDepth, const float alpha)
{
   ...
   // Build the corners
   buildDisc(geom, color, maxPt, innerRad, outerRad, numSegs, pi * 0.0f,
             pi * 0.5f, frontDepth, backDepth, true, alpha);
   buildDisc(geom, color, ul_corner, innerRad, outerRad, numSegs, pi * 0.5f,
             pi * 1.0f, frontDepth, backDepth, true, alpha);
   buildDisc(geom, color, minPt, innerRad, outerRad, numSegs, pi * 1.0f,
             pi * 1.5f, frontDepth, backDepth, true, alpha);
   buildDisc(geom, color, lr_corner, innerRad, outerRad, numSegs, pi * 1.5f,
             pi * 2.0f, frontDepth, backDepth, true, alpha);

   // Build edges
   buildRectangle(geom, color, OSG::Pnt2f(minPt.x()  -outerRad, minPt.y()),
                  OSG::Pnt2f(minPt.x() - innerRad, maxPt.y()), frontDepth,
                  backDepth, alpha);  //left
   buildRectangle(geom, color, OSG::Pnt2f(maxPt.x() + innerRad, minPt.y()),
                  OSG::Pnt2f(maxPt.x() + outerRad, maxPt.y()), frontDepth,
                  backDepth, alpha);  //right
   buildRectangle(geom, color, OSG::Pnt2f(minPt.x(), minPt.y() - outerRad),
                  OSG::Pnt2f(maxPt.x(), minPt.y() - innerRad), frontDepth,
                  backDepth, alpha);  //bottom
   buildRectangle(geom, color, OSG::Pnt2f(minPt.x(), maxPt.y() + innerRad),
                  OSG::Pnt2f(maxPt.x(), maxPt.y() + outerRad), frontDepth,
                  backDepth, alpha);  //top
                          ...
                          const bool capIt, const float alpha)
{
   ...
   for ( unsigned int strip = 0; strip < 4; ++strip )
   {
      ...
      // Skip inner if inner rad is 0 (since it would just be a bunch of tris
      ...
      {
      case 0:  // Front
         bottom_depth = frontDepth;
         top_depth    = frontDepth;
         bottom_verts = &inner_edge;
         top_verts    = &outer_edge;
         break;
      case 1:  // Outer
         bottom_depth = frontDepth;
         top_depth    = backDepth;
         bottom_verts = &outer_edge;
         top_verts    = &outer_edge;
         break;
      case 2:  // Back
         bottom_depth = backDepth;
         top_depth    = backDepth;
         bottom_verts = &outer_edge;
         top_verts    = &inner_edge;
         break;
      case 3:  // Inner
         bottom_depth = backDepth;
         top_depth    = frontDepth;
      ...
      {
         OSG::Vec3f bottom_vert((*bottom_verts)[i].x(),
                                (*bottom_verts)[i].y(), bottom_depth);
         OSG::Vec3f top_vert((*top_verts)[i].x(), (*top_verts)[i].y(),
                             top_depth);

         // Calculate normal
         // - On last set of verts we do it different because we have no next
         //   vert
         OSG::Vec3f normal;
         if ( i != (*bottom_verts).size() - 1 )
         {
            OSG::Vec3f next_bottom_vert((*bottom_verts)[i + 1].x(),
                                        (*bottom_verts)[i + 1].y(),
                                        bottom_depth);

            // Case with verts on top of each other at center of disc
            if ( next_bottom_vert == bottom_vert )
            {
               next_bottom_vert = OSG::Vec3f((*top_verts)[i + 1].x(),
                                             (*top_verts)[i + 1].y(),
                                             top_depth);
            }
            normal =
               (top_vert - bottom_vert).cross(next_bottom_vert - top_vert);
         }
         else
         {
            OSG::Vec3f prev_bottom_vert((*bottom_verts)[i - 1].x(),
                                        (*bottom_verts)[i - 1].y(),
                                        bottom_depth);

            // Case with verts on top of each other at center of disc
            if ( prev_bottom_vert == bottom_vert )
            {
               prev_bottom_vert = OSG::Vec3f((*top_verts)[i - 1].x(),
                                             (*top_verts)[i - 1].y(),
                                             top_depth);
            }
            normal =
               (top_vert - prev_bottom_vert).cross(bottom_vert - top_vert);
         }
         normal.normalize();
   ...
   if ( capIt && ! only_front_flag )
   {
      // Start
      {
         ...
         OSG::Pnt3f front_bottom(inner_edge[0].x(), inner_edge[0].y(),
                                 frontDepth);
         OSG::Pnt3f front_top(outer_edge[0].x(), outer_edge[0].y(),
                              frontDepth);
         OSG::Pnt3f back_bottom(inner_edge[0].x(), inner_edge[0].y(),
                                backDepth);
         ...
         verts->addValue(back_bottom);
         verts->addValue(back_top);
         verts->addValue(front_bottom);
         verts->addValue(front_top);
         norms->addValue(normal);
         norms->addValue(normal);
         norms->addValue(normal);
         norms->addValue(normal);
      ...
      // End
      {
         ...
         OSG::Pnt3f front_bottom(inner_edge.back().x(), inner_edge.back().y(),
                                 frontDepth);
         OSG::Pnt3f front_top(outer_edge.back().x(), outer_edge.back().y(),
                              frontDepth);
         OSG::Pnt3f back_bottom(inner_edge.back().x(), inner_edge.back().y(),
                                backDepth);
         OSG::Pnt3f back_top(outer_edge.back().x(), outer_edge.back().y(),
                             backDepth);
...
{
   OSG::TextTXFFace* new_face =
      OSG::TextTXFFace::create(mFamilyName, mStyle, mParams);

   if ( NULL == new_face )
   {
      throw Exception("ERROR: Failed to allocate face.", VRKIT_LOCATION);
   }
                          ...
                          const float scale, const float spacing)
{
   ...
   // Compute extra offset since we are passing in offset of upperleft and
   // OpenSG wants lower left of first line.
   float line_height = layout_result.textBounds.y() / float(lines.size());
   line_height *= scale;
   OSG::Vec2f single_line_offset(0, line_height);

   font.mFace->fillGeo(geom, layout_result, scale, offset - single_line_offset,
                       color);

   OSG::ChunkMaterialPtr mat =
#if OSG_MAJOR_VERSION < 2
      OSG::ChunkMaterialPtr::dcast(geom->getMaterial());
#else
      OSG::cast_dynamic<OSG::ChunkMaterialPtr>(geom->getMaterial());
#endif
   vprASSERT(OSG::NullFC != mat);

   OSG::StateChunkPtr tex_state_chunk =
      mat->find(OSG::TextureChunk::getClassType());
   OSG::TextureChunkPtr tex_chunk =
#if OSG_MAJOR_VERSION < 2
      OSG::TextureChunkPtr::dcast(tex_state_chunk);
#else
      OSG::cast_dynamic<OSG::TextureChunkPtr>(tex_state_chunk);
#endif
                        ...
                        const float scale, const float spacing)
{
   ...
   font.mFace->addToGeom(geom, layout_result, scale,
                         offset - single_line_offset, color);
}
