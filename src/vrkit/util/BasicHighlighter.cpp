...
BasicHighlighter::BasicHighlighter()
   ...
   , mChooseHighlightID(GeometryHighlightTraverser::HIGHLIGHT1)
...
BasicHighlighterPtr BasicHighlighter::init(ViewerPtr viewer)
{
   ...
   {
      fs::path vrkit_base_dir_path(vrkit_base_dir, fs::native);
      fs::path shader_subdir_path("share/vrkit/data/shaders");
      mShaderSearchPath.push_back(vrkit_base_dir_path / shader_subdir_path);
   }
   ...
   if ( mEnableShaders )
   {
      try
      {
         ...
         OSG::Vec3f isect_color_vec(mIntersectColor[0], mIntersectColor[1],
                                    mIntersectColor[2]);
         GeometryHighlightTraverser::uniform_map_t isect_uniform_params;
         ...
         OSG::Vec3f choose_color_vec(mChooseColor[0], mChooseColor[1],
                                   mChooseColor[2]);
         GeometryHighlightTraverser::uniform_map_t choose_uniform_params;
         ...
         OSG::Vec3f grab_color_vec(mGrabColor[0], mGrabColor[1],
                                   mGrabColor[2]);
         GeometryHighlightTraverser::uniform_map_t grab_uniform_params;
...
event::ResultType BasicHighlighter::objectIntersected(SceneObjectPtr obj,
                                                      gmtl::Point3f)
{
...
event::ResultType BasicHighlighter::objectDeintersected(SceneObjectPtr obj)
{
...
event::ResultType
BasicHighlighter::objectChoiceChanged(const std::vector<SceneObjectPtr>& objs,
                                      const bool added)
{
...
event::ResultType
BasicHighlighter::objectsSelected(const std::vector<SceneObjectPtr>& objs,
                                  const bool selected)
{
...
event::ResultType BasicHighlighter::objectPicked(SceneObjectPtr obj,
                                                 const bool picked)
{
   // Switch from the intersection highlight to the grab highlight.
   if ( picked )
...
bool BasicHighlighter::isIntersected(SceneObjectPtr obj)
{
   return std::find(mIntersectedObjs.begin(), mIntersectedObjs.end(), obj) != mIntersectedObjs.end();
}

bool BasicHighlighter::isChosen(SceneObjectPtr obj)
{
   return std::find(mChosenObjs.begin(), mChosenObjs.end(), obj) != mChosenObjs.end();
}

bool BasicHighlighter::isGrabbed(SceneObjectPtr obj)
{
